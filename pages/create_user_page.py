...
class CreateUserPage(BasePage):
    def create_new_user(self, password, gender, date_of_birth,
                        city_in_usa):
        if gender == "MR":
            mr_radiobutton = self.find_element(
                RegistrationPageLocators.MR_RADIOBUTTON_LOCATOR)
            mr_radiobutton.click()
        elif gender == "MRS":
            mrs_radiobutton = self.find_element(
                RegistrationPageLocators.MRS_RADIOBUTTON_LOCATOR).click()
            mrs_radiobutton.click()
        first_name = self.find_element(
            RegistrationPageLocators.FIRST_NAME_FIELD_LOCATOR)
        first_name.send_keys(constans.FIRSTNAME_LOGIN)
        last_name = self.find_element(
            RegistrationPageLocators.LAST_NAME_FIELD_LOCATOR)
        last_name.send_keys(constans.LASTNAME_LOGIN)
        password_field = self.find_element(
            RegistrationPageLocators.PASSWORD_FIELD_LOCATOR)
        ...
        days_of_birth_dropdown.select_by_value(birth_date[0])
        ...
        months_of_birth_drop_down.select_by_value(birth_date[1])
        ...
        years_of_birth_drop_down.select_by_value(birth_date[2])
        address = self.find_element(
            RegistrationPageLocators.ADDRESS_FIELD_LOCATOR)
        ...
        time.sleep(0.1)
        state_drop_down.select_by_visible_text(city_in_usa)
        zip_code = self.find_element(
            RegistrationPageLocators.ZIP_CODE_FIELD_LOCATOR)
        zip_code.send_keys(constans.ZIPCODE_LOGIN)
        mobile_phone = self.find_element(
            RegistrationPageLocators.MOBILE_PHONE_FIELD_LOCATOR)
        mobile_phone.send_keys(constans.MOBILE_PHONE_LOGIN)
        register_button = self.find_element(
            RegistrationPageLocators.REGISTER_BUTTON_LOCATOR)
        register_button.click()
        finish_registration = self.find_element(
            LoginPageLocators.WELCOME_TO_ACCOUNT_TEXT)
        return finish_registration, LoginPage(self.driver,
